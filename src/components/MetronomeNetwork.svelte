  ...
  try {
    ...
    socket.addEventListener("message", (event: MessageEvent) => {
      try {
        ...
        if (message.type === "registered") {
          ...
          console.log(`Successfully registered with server as client: ${message.clientId}`);
        ...
        if (message.type === "stateUpdate" && message.clientId !== networkState.clientId) {
    ...
    socket.addEventListener("close", () => {
      ...
      if (networkState.connectionAttempts < 5) {
        const backoffTime = Math.min(1000 * (2 ** networkState.connectionAttempts), 30000);
        networkState.connectionAttempts++;
        
        console.log(`Attempting to reconnect in ${backoffTime}ms (attempt ${networkState.connectionAttempts})`);
 ...
 */
const debouncedStateUpdate = (): void => {
