...
let lastReceivedState = $state<MetronomeState | undefined>(undefined)

onMount(() => {
	peer.subscribe("metronomeState", (from, data) => {
		lastReceivedState = (data as { state: MetronomeState }).state
		metronomeState = lastReceivedState
	})
})

$effect(() => {
	if (lastReceivedState !== metronomeState) {
